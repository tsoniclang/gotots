...
const hex = (value: GoString): string =>
    Array.from(value.text(), byte => byte.charCodeAt(0).toString(16).padStart(2, "0")).join("");
...
console.log(hex(values.MaxString(GoString.fromText("a"), GoString.fromText(String.fromCharCode(255)), GoString.fromText("z"))));
console.log(hex(values.MinString(GoString.fromText("a"), GoString.fromText(String.fromCharCode(255)), GoString.fromText("z"))));
...
console.log(values.ConstantString().text());
const [value, order] = values.OrderedMax();
console.log(String(value), order.text());
const [prerequisiteValue, prerequisiteOrder] = values.OrderedWithPrerequisite();
console.log(String(prerequisiteValue), prerequisiteOrder.text());
`
