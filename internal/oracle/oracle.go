...
// termination.
//
// The fixture protocol: one package named fixture whose exported
// zero-parameter functions are the cases. Each case's results (or its
// panic) are printed one line per case in sorted name order by generated
...
// Translate materializes fixtureSource as a Go module in workDir, loads
// it through the shared typed loader, and runs the translation pipeline.
func Translate(workDir, fixtureSource string) (*translate.Generated, *packages.Package, error) {
	...
	if err := os.MkdirAll(filepath.Join(workDir, "fixture"), 0o755); err != nil {
		return nil, nil, err
	}
	if err := os.WriteFile(filepath.Join(workDir, "fixture", "fixture.go"), []byte(fixtureSource), 0o644); err != nil {
		return nil, nil, err
	}

	prof := &profile.Profile{
		GoModule:   "oracle.fixture",
		OwnedRoots: []string{"fixture"},
	}
	...
	for _, p := range loaded {
		if p.PkgPath == "oracle.fixture/fixture" && typedload.RoleOf(p, workDir) == typedload.RoleProduction {
			fixture = p
		}
	}
	if fixture == nil {
	...
	generated, err := translate.Package(fixture, workDir, translate.Options{
...
// Run translates fixtureSource (the complete fixture package file) and
// executes both sides. The Node toolchain is required: its absence blocks
// the gate rather than skipping it.
func Run(workDir, fixtureSource string) (*Result, error) {
	resolved, err := bootstrapOnce()
	if err != nil {
		return nil, err
	}
	...
	generated, fixture, err := Translate(workDir, fixtureSource)
	if err != nil {
		return nil, err
	}
