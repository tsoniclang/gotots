...
func TestNamedResultDeferMutationInClosureWithheld(t *testing.T) {
	// Top-level functions take the named-exit lowering; a CLOSURE with
	// named results and defers still fails closed (the break cannot
...
`})
	if err == nil {
		t.Fatal("expected deferred named-result mutation in a closure to be withheld")
	}
