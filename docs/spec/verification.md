...
Each dependency-ordered implementation step is a separately certified
architecture boundary. The exact revision must pass its complete applicable
proof battery before a dependent step starts:

1. focused positive, negative, and cross-product contract tests at the truth
   owner and every migrated consumer;
2. an independently derived exact multiset join over the changed source,
   semantic, plan, AST, artifact, or execution identities;
3. production-path mutations that demonstrate every claimed failure class is
   rejected by its owning gate with exact identities;
4. inspection of ordinary real artifacts, adversarial fixtures, and the
   twenty-item affected tail rather than summaries alone;
5. structural/staticness walls plus source/generated bytes, record widths,
   peak RSS, wall time, typecheck, and runtime bounds with parent deltas where
   the step can affect them;
6. a guarded broader suite that exercises every migrated producer and consumer;
   and
7. broad source/artifact searches proving the superseded owner, schema,
   fallback, compatibility route, stale test, and stale prose are absent.

The evidence is recorded against one clean revision and its exact inputs.
Focused success does not waive a cost gate, synthetic evidence does not replace
a real artifact, phase exit does not retroactively certify an earlier step, and
an unchanged earlier result is rerun whenever the changed boundary can affect
it. Any failure keeps that step open and blocks downstream implementation.
...
|---|---|
| 01 Input integrity | clean source/compiler trees, exact tool/config fingerprints including the pinned TS-Go revision/schema/generator manifest, focused/full Go tests, bounded runner health |
...
| 05 Independent parity | independently derived definition semantics and occurrence resolutions match typed Go evidence; later the pinned TS-Go parser independently reparses generated files and the resulting schema-level declarations and operation shapes match the constructed TS-Go AST, typed Go evidence, and public plan |
...
| 07 Artifact ownership | implementations, generated TS-Go AST units, files, definitions, markers, hashes, and provenance join one-to-one; every target node is schema-admitted and every formatted byte is owned by that tree or the sole formatter |
...
| 10 Strict TypeScript | every materialized runtime, stdlib, external, generated, manual, and extension module parses through the pinned TS-Go service, resolves/typechecks together, and satisfies the Tsonic target subset |
...
The expected-type matrix includes a function literal returned as a named
function type:
...
The return operation must carry exactly one assignment conversion whose site
is the literal occurrence, whose source is the literal's unnamed signature,
and whose target is `Sequence`. The independent checker derives the result
type from the parent return occurrence while deriving the literal signature
from the child definition. A mutation that asks the child definition for the
parent result signature must fail the exact implicit-effect join.

The capture matrix includes:
...
The nested definition must occur exactly once in `value.CapturedBy` and zero
times in `T.CapturedBy`; `T` remains referenced by canonical semantic type
identity. A mutation that admits captures for every binding role fails both
the binding constructor and the independent capture join.

The binding-order matrix places an implicit import before an explicit import
alias in one file scope and two unnamed parameters in one signature scope.
Each unnamed parameter exact-joins one checker object across
`Info.Implicits`, its checker signature tuple slot, and its unnamed
`ast.Field`. It must not use the invalid premise that Go permits named and
unnamed parameters in one list: `func f(int, named string)` defines two named
`string` parameters. Both producer and independent verifier derive one
complete structural order before projection. Mutations that break the
three-way signature join, number explicit and implicit bindings separately,
number only selected bindings, accept equal ordering evidence, or restore
`types.Object.Pos` must fail through an absent binding, identity collision,
wrong stable ordinal, or independent-join difference.
...
arguments and exact-join every reference to one origin member target. Aliases
of the owner must produce the target owner's identity, not an alias-owned
identity. Mutating method-origin, alias-target, promoted-owner, or field-owner/
ordinal canonicalization produces a missing/wrong-target failure.

The compile-time-in-executable matrix includes
`values := [...]int{1, 2}`,
`values := [base + 1]int{}`, and
`const limit = base + 1` inside a function. The ellipsis must resolve to the
independently derived `[2]int` semantic type through the exact
`ArrayType.Len` parent. The explicit array-length and constant-initializer
roots and descendants must independently re-derive compile-time context,
carry exact declaration/type coverage, and own no runtime operation. A control
`value := base + 1` must remain a binary runtime operation. Replacing coverage,
accepting a non-array/non-const context, dropping an occurrence, or inventing a
runtime operation fails the resolution/type/operation conservation gates.
...
occurrence, while both resolve to one canonical type-owned member target and
the package contains zero standalone field records. A second package spelling
the same anonymous struct resolves the same target without either package
claiming member ownership. Equivalent anonymous-interface methods receive the
same treatment. Mutations that serialize a member declaration, choose one
arbitrary spelling, create one record per spelling/package, omit the owner
type, or change component class/name/visibility package/field ordinal/method
signature fail target conservation, relocation, or provider-projection parity.

A predeclared fixture resolves `err.Error()` through the `error` named/interface
type component while `builtin` contains only the standalone `error` type
declaration. It must pass through the same member-target resolver as ordinary
named and anonymous interfaces. A builtin-specific `Error` insertion, missing
owner type, or globally package-owned member mutation fails.
...
binder role, and ordinal. An imported-callable fixture equivalent to
`func F[T any]() { reflect.TypeFor[T]() }` proves the referenced object is
registered before its generic selector signature is checked even though the
imported metadata record carries no `Types()` node from the selective
hydration graph. Reversing that dependency order or restoring a
whole-universe package-scope index must fail. Mutations that key a type parameter by
...
Normalized-package mutations additionally cross an identity-domain reference,
use zero and out-of-range references, shift and overlap a relation range,
duplicate or unsort an identity dictionary entry, select a payload whose tag
does not own it, omit or retain an inactive payload, repeat a portable identity
...
excluded enclosing occurrence has no semantic resolution; changing an actual
operand, initializer, or implicit-operation site to that unresolved occurrence
must still fail.

The pre-seal semantic-closure matrix independently exercises roots introduced
by a definition, structural/declaration resolution, binding type, selected
object, method selection, generic instance, implicit conversion, and canonical
type descriptor. It follows both type-to-type and type-to-declaration edges.
Mutations that omit any root class, omit a type-owned declaration edge, select
only declarations named directly by definitions, apply mixed-depth filtering
to provider production, restore a post-seal local-package projection, or
retain a package-wide public-record pool must fail the closure, artifact-
admission, or architecture wall with exact missing/extra identities.
...
- one visit per retained local occurrence by each closed, separately reported
  occurrence pass—context, object occurrence, implicit-binding selection,
  capture, and resolution—and one separately reported direct pass over each
  checker definition, implicit-definition, and scope relation required for
  complete-set identity ordering, with no hidden peer, scope, definition, or
  ancestry scan;
...
- canonical sorts, binary searches, deduplication, and validation joins compare
  typed identity components without calling identity `String()` or retaining
  rendered keys; comparator-law tests cover zero and every identity form,
  numeric-offset fixtures distinguish structural from decimal lexical order,
  allocation tests require zero allocations per comparison, and a production
  source mutation restoring a string comparator fails the architecture wall;
- exact definition, operation, and type byte attribution is observed by the
  sole shard writer during its one authoritative serialization; each
  category's top-twenty set stays bounded at twenty while records stream, and
  no metrics/reporting path re-encodes a semantic record, retains an
  all-record size list, or sorts one merely to discard all but twenty;
- manifests plus one final logical semantic package, never accumulated decoded
  local packages or all provider semantic shards, as the ordinary residency
  bound; local-only, provider-only, and mixed-authority projections report
  separate shard loads and the same maximum-one-package residency invariant;
- no eager all-package frontend input graph: a structural surface gate rejects
  resident slices/maps of package inputs, its negative control proves that the
  rejected shape is detectable, and a mutation restoring eager package
  preparation must fail before semantic projection; package-scoped
  executable-occurrence lookup is mutation-proved against restoring one
  complete-set scan per package;
- streaming projection with zero package-wide encoded or wire-tree
  materialization, zero immutable-package clone, no simultaneous complete
  local/provider/merged package set, exact manifest-capacity agreement, and
  peak RSS below the frozen largest-package and whole-compilation budgets;
  a structural surface gate rejects any immutable semantic-package method
  returning a record slice, with a negative control proving the rejected
  shape is detectable, and the largest-package verifier uses ordered visitors
  or canonical keyed lookups rather than snapshot maps or copied record
  classes;
  the phase-exit fixture projects the largest real package in the selected Go
  toolchain three times, proves that each released projection leaves no
  monotonic retained-package growth, and runs the complete self-host
  compilation under a `4 GiB` peak-RSS ceiling; exceeding that product ceiling
  reopens ownership, lifecycle, or artifact grain rather than authorizing a
  second representation;
- one typed identity dictionary entry per unique package-local portable
  identity, compact stored record widths bounded independently of the largest
  inactive sum-variant payload, exactly one active payload per closed record,
  and total normalized storage attributed as dictionaries, record cores,
...
The independent verifier parses final TypeScript with the exact pinned TS-Go
parser/compiler service and extracts the same schema-level AST contract used
by construction. Before implementation, every schema field is classified once
as structural, parser-derived source range/trivia, synthetic-only metadata, or
formatter-normalized parentheses. The verifier exact-joins the structural
projection—node kinds, structural fields, optionality, child order,
declarations, types, fields, methods, heritage, generic binders, calls, control
forms, imports, ownership markers, and planned operation annotations—against
the validated constructed-tree projection. The field classification is closed,
schema-derived, and mutation-tested; it is not a per-node exception list. The
verifier does not call lowering, the formatter, a generated-tree
canonicalizer, or semantic canonicalization.
...
Before that semantic sweep, an AST-authority gate rejects any target node not
constructible from the generated pinned-schema bindings, any handwritten
target kind/field/factory/visitor table, any generic map/property-bag AST,
ESTree or TypeScript-compiler-object construction, and any raw TypeScript
fragment carried through lowering. Scalar identifier, literal, comment, and
module-specifier payloads may contain arbitrary text; mutation tests prove they
are escaped as data by their owning field and are never parsed, interpolated,
or otherwise reinterpreted as a subtree.
...
- no target-node authority exists outside `schema/tsgo/` and its generated
  bindings, no handwritten kind/field/factory/visitor table or alternate AST
  survives, and no formatter input can bypass generated AST validation;
...
definition, erase an interface payload, emit an oversized per-target switch,
change a pinned TS-Go node kind/field/optionality/child order, hand-add a target
factory, return a source string from lowering, inject a raw-source node, and
introduce a second formatter. Package-universe mutations include
import-spelling classification, module-less standard rejection,
acquisition/provenance conflation, and omitted dependency closure. The
appropriate gate must fail each mutation.
...
toolchain, pinned TS-Go revision and schema/generator digests, target compiler,
runtime, standard library, contracts, extensions, tests, reports, and immutable
publication root. When stored in version control, the evidence-only commit's
parent is exactly the attested implementation revision. Stale, ancestor-only,
dirty-tree, or non-reproducible evidence is invalid.
