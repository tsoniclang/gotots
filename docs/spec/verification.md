...
Stateful provider representation verification derives the complete recursively
retained named-interface set from each profiled selected Go concrete type and
exact-joins it to the profile certificate. It independently verifies the
ordinary and alternate type/value exports, every supplied source method and
target member, all method effects, implementation owners, fingerprints, and
the profile key built from current canonical interface-method facets. It also
proves that constructor profiles returning the named type select the same
profile key and exact target representation as the generated alias artifact.

A focused `bufio.Reader` fixture covers a generated reader whose `Read` blocks,
partial reads, buffered follow-up reads, EOF, one hundred empty reads,
`ReadByte`, `ReadBytes`, nil input, concrete storage in a field, return through
another function, conversion to `io.Reader`, and repeated calls preserving
state. Artifact inspection must show one stable generated type/value alias,
one certified profile implementation, direct static calls, and no cast,
union, runtime Promise test, eager source drain, per-use wrapper, or direct
consumer import of the private facet. Go and strict-ESM TypeScript execute
differentially under both the ordinary and cooperative profile cases.

Mutations remove or add one retained interface, change one interface-method
effect, omit one supplied concrete method, redirect one target member, select
the ordinary constructor with the cooperative represented type, bypass the
generated alias, return a synchronous provider error from the cooperative
profile, retain a stale profile after its final consumer disappears, and
restore a direct private-facet import. Each must fail at certification,
artifact dependency/conservation, strict typechecking, differential execution,
or the broad ownership search. Generated bytes, AST nodes, typecheck wall/RSS,
runtime, alias/profile counts, and the twenty largest affected artifacts are
reported with parent deltas.
...
Provider-created canonical result interfaces have a separate exact join. The
gate proves every `implementedResultInterfaces` identity is a direct named
canonical result, is also a runtime contract interface, and is implemented by
the certified target export. A blocking-input fixture must force one target
method from synchronous to cooperative, reconstruct the canonical
interface-method contract, await every affected call, and then converge with
zero dirty artifacts. A pass-through `error` control must remain governed by
its source effect and must not be elevated merely because it appears in
`contractInterfaces`. Mutations that remove implemented-result ownership,
move it to a parameter or nested value, change the target method effect,
downgrade a cooperative source method, omit the reverse dependency, or reuse
the pre-elevation profile key must fail at seed validation, certification,
selection, convergence, strict typechecking, or the Go/TypeScript
differential.

Required semantic-protocol profiles have a separate non-vacuous matrix. The
`errors.Is` fixture covers nil, direct comparable equality, a custom
`Is(error) bool`, single and multi-error unwrapping in depth-first order, and a
non-comparable target. It runs Go versus strict generated ESM and inspects the
typed anonymous-interface guards and calls. The same method sets are exercised
with synchronous and cooperative implementations so the profile key and outer
effect change only with exact selected method effects. Certification mutations
remove a protocol, change its method name, parameter/result expression,
selected effect, target member, target fingerprint, or required-selection bit;
each must fail before execution. Emission mutations bypass the profile when the
public ABI matches, pass a raw value instead of a canonical predicate, call an
optional member without narrowing, or recover a member through target shape;
each must fail selection, strict typechecking, artifact shape, or the
differential.

Dynamic-comparability proof inspects the shared token for comparable and
non-comparable generated values, provider values, and provider bridges. The
non-comparable `errors.Is(target, target)` case returns false without invoking
the adapter's panicking equality method, while ordinary interface equality for
the same dynamic value still panics as Go requires. Mutations hard-code the
token true, add a per-instance/provider-only flag, catch an equality panic,
drop comparability through a bridge, or omit the property from any runtime
interface-value implementation; each fails strict typechecking, token-shape,
source-size, or differential evidence. Token and adapter counts and bytes are
reported separately; adding comparability must not add one field or helper per
boxed value or call.
...
A stateful provider-profile gate inspects every direct exported Go field and
its produced target member. It mutates field omission, an extra public target
field, name, visibility, direct ordinal, embedding, represented type, owner,
and fingerprint. Omission and extras fail the profile-surface exact join;
source-contract mutations fail manifest validation; represented-type drift
fails strict typechecking of a generated field read and write. An embedded
field fixture proves a promoted selector emits the actual nested field path,
never a flattened duplicate member.
