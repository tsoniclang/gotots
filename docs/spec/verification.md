...
|---|---|---|
| integers | every admitted width/profile/operator/conversion at boundary values | width alias collapse; unsafe number literal; missing narrowing/bounds operation |
...
5. the runtime export request exact-joins one definition and fails for omitted,
   duplicated, or wrong-module symbols;
6. generated bytes and target AST nodes are measured for 1x/2x/4x use sites;
